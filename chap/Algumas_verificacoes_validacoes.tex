...
Uma vez que o modelo constitutivo acoplado (EPVP) está generalizado para o caso tridimensional é interessante fazer uma análise paramétrica considerando algum aspecto que não pode ser analisado com o modelo axissimétrico. O aspecto escolhido nessa análise foi a razão entre o raio horizontal e vertical. Dessa forma, foram considerados três razões de aspecto: 1 (R1), ou seja, circular, 1.5 (R1,5) e 2 (R2). Além disso, os cálculos foram executados sem revestimento (S/R) e com revestimento (C/R) afastado de $d0=4L_p$ da face de escavação. As propriedades do maciço são: $E=1500$MPa, $\nu=0.498$, $c^i=c^p=c^r =4\sqrt{3}/2$MPa, $c^{vp}=3\sqrt{3}/2$MPa, $\eta = 4 \cdot 10^4$dia, $n=1$, $f_0=1$MPa e $p_v=p_h=9$MPa. E do revestimento: $E_{rev}=3000$MPa e $\nu_{rev}$=0,3. Para evidenciar a diferença que o modelo acoplado pode representar os resultados foram comparados com modelo viscoplástico (VP) com uma coesão de $4\sqrt{3}/2$.

Como a seção transversal é eliptica as convergências são medidas ao longo do maior e menor eixo da seção, portanto,  tem-se $U_y = -u_y(r=R_{vi})/R_{vi}$ e $U_x = -u_x(r=R_{hi})/R_{hi}$. A \autoref{VP-FINAL-ESCAVACAO-UY} e \autoref{EPVP-FINAL-ESCAVACAO-UY} mostram os perfis de convergências na direção $y$ para ambos os modelos (VP) e (EPVP) no final da escavação do túnel. O valor numérico da convergência no equilíbrio está na legenda.

\begin{figure}[H]
	\begin{center}
		\includegraphics[scale = 1.0]{0727-VP-FINAL-ESCAVACAO-UY.pdf
		}
	\end{center}
	\caption{\label{VP-FINAL-ESCAVACAO-UY}Perfis de convergência na direção y ao final da escavação sem e com revestimento para diferentes razões de aspecto da seção transversal para o modelo viscoplástico}
\end{figure}

\begin{figure}[H]
	\begin{center}
		\includegraphics[scale = 1.0]{0728-EPVP-FINAL-ESCAVACAO-UY.pdf
		}
	\end{center}
	\caption{\label{EPVP-FINAL-ESCAVACAO-UY}Perfis de convergência na direção y ao final da escavação sem e com revestimento para diferentes razões de aspecto da seção transversal para o modelo elastoplástico-viscoplástico}
\end{figure}

Pode-se notar, como esperado, que as convergências do modelo acoplado foram maiores que o modelo viscoplástico. A \autoref{VP-FINAL-ESCAVACAO-UX} e \autoref{EPVP-FINAL-ESCAVACAO-UX} mostram os perfis de convergências na direção $x$ para ambos os modelos (VP) e (EPVP) no final da escavação do túnel.

\begin{figure}[H]
	\begin{center}
		\includegraphics[scale = 1.0]{0729-VP-FINAL-ESCAVACAO-UX.pdf
		}
	\end{center}
	\caption{\label{VP-FINAL-ESCAVACAO-UX}Perfis de convergência na direção x ao final da escavação sem e com revestimento para diferentes razões de aspecto da seção transversal para o modelo viscoplástico}
\end{figure}

\begin{figure}[H]
	\begin{center}
		\includegraphics[scale = 1.0]{0730-EPVP-FINAL-ESCAVACAO-UX.pdf
		}
	\end{center}
	\caption{\label{EPVP-FINAL-ESCAVACAO-UX}Perfis de convergência na direção x ao final da escavação sem e com revestimento para diferentes razões de aspecto da seção transversal para o modelo elastoplástico-viscoplástico}
\end{figure}
As convergências ainda foram maiores para o modelo (EPVP), contudo, houve uma inversão. As maiores convergências ocorreram com razão de aspecto menor. Isso ocorre pois a convergência é dividida pelo raio horizontal que está maior. Se multiplicar a convergência pelo raio, tem-se uma maior deformação para uma maior razão de aspecto.
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[scale = 1.0]{0731-VP-LONGO-PRAZO-UY.pdf
		}
	\end{center}
	\caption{\label{VP-LONGO-PRAZO-UY}Perfis de convergência na direção y no longo prazo sem e com revestimento para diferentes razões de aspecto da seção transversal para o modelo viscoplástico}
\end{figure}

\begin{figure}[H]
	\begin{center}
		\includegraphics[scale = 1.0]{0732-EPVP-LONGO-PRAZO-UY.pdf
		}
	\end{center}
	\caption{\label{EPVP-LONGO-PRAZO-UY}Perfis de convergência na direção y no longo prazo sem e com revestimento para diferentes razões de aspecto da seção transversal para o modelo elastoplástico-viscoplástico}
\end{figure}

\begin{figure}[H]
	\begin{center}
		\includegraphics[scale = 1.0]{0733-VP-LONGO-PRAZO-UX.pdf
		}
	\end{center}
	\caption{\label{VP-LONGO-PRAZO-UX}Perfis de convergência na direção x no longo prazo sem e com revestimento para diferentes razões de aspecto da seção transversal para o modelo viscoplástico}
\end{figure}

\begin{figure}[H]
	\begin{center}
		\includegraphics[scale = 1.0]{0734-EPVP-LONGO-PRAZO-UX.pdf
		}
	\end{center}
	\caption{\label{EPVP-LONGO-PRAZO-UX}Perfis de convergência na direção x no longo prazo sem e com revestimento para diferentes razões de aspecto da seção transversal para o modelo elastoplástico-viscoplástico}
\end{figure}

A tabela XX resume a razão entre as convergências.
...
\begin{table}[H]
	...
	\normalsize
\end{table}
A tabela 7.1 mostra a relevância que um modelo acoplado pode ter nas análises de túneis profundos. Sem revestimento um aumento de 31\% (no curto prazo) a 57\% (no longo prazo) em relação ao modelo viscoplástico. Com o revestimento os valores ficaram entre 10\% e 20\%.
\begin{table}[H]
