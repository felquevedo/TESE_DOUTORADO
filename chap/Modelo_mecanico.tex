...
\begin{equation}
	...
	f(\sigmall,\ql) = f(I_1,J_2,\theta,c,\phi) = \dfrac{\sin(\phi)}{3}I_1 + \left(\cos(\phi)-\dfrac{1}{\sqrt{3}}\sin(\theta)\sin(\phi)\right)\sqrt{J_2}-c\cos(\phi) 
\end{equation}
...
\end{equation}
Igualando-se as expressões (\ref{eq:f_von-Mises}) e (\ref{eq:f_Tresca}) à zero e isolando $\sqrt{J_2}$ é possível relacionar ambas as superfícies através da coesão (Figura 5.7):
\begin{equation}
	\label{eq:f_von-Mises}
	c_{TR} = \dfrac{2}{\sqrt{3}}\cos(\theta)c_{VM} = \left\{
	\begin{array}{lcl}
		\dfrac{2}{\sqrt{3}}c_{VM}, ~~~\text{se}~\theta = 30^\circ \\ 
		c_{VM},~~~~~~~~~\text{se}~\theta = 0^\circ
	\end{array}
	\right.
\end{equation}
\begin{figure}[H]
...
\end{figure}
Tal como a função de escoamento, o potencial plástico geralmente é descrito através dos invariantes do tensor de tensões e pode ser determinado aplicando a regra da cadeia em (\ref{eq:vetor_fluxo_elastoplastico}). Por exemplo, se $g(I_1,\sqrt{J_2},J_3,\ql)$, tem-se (adaptado de \citeauthor{Viladkar1995}, \citeyear{Viladkar1995}, p. 333):
\begin{equation}
	\label{eq:invariantes_tensores}
	\begin{array}{lcl}
	...
	\end{array}
\end{equation}
Nessa descrição, apenas as constantes $C_1$, $C_2$  e $C_3$ são particularidades das superfícies potenciais, sendo que para a superfície de Mohr-Coulomb tem-se:
\begin{equation}
	\label{eq:c1MC}
...
A lei de endurecimento/amolecimento caracteriza, em geral, a dependência das variáveis internas (por exemplo, a coesão) em relação ao histórico de deformações plásticas. Essa lei de endurecimento/amolecimento é postulada da seguinte forma:
\begin{equation}
...
\begin{equation}
	...
	\dot f = \dfrac{\partial f}{\partial \sigmall}:\sigmall + \dfrac{\partial f}{\partial \ql}\cdot \dot \ql = \dfdsll:\dot \sigma + \dfdql \cdot \dot \ql = 0
\end{equation}
A equação (\ref{eq:consistencia}) é conhecida como condição de consistência.
...
\begin{equation}
	\label{eq:Dep}
	\Dllll^{ep} = \Dllll - \dfrac{\left(\Dllll:\dgdsll \right)\otimes\left(\Dllll:\dfdsll \right)}{\dfdsll:\Dllll:\dgdsll-\dfdql \cdot \hl}
\end{equation}
