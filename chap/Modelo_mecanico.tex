...
\end{figure}
Em um primeiro momento, no presente trabalho, não é considerado a influência da lei de endurecimento e as variáveis internas serão constantes ao longo do histórico de deformações, ou seja, $\dot \ql = \underline 0$. Nesse caso, tem-se a elastoplasticidade perfeita. Considerando apenas o endurecimento/amolecimento isotrópico, nos modelos adotados, isso significa que a coesão e o ângulo de atrito permanecem constantes durante a deformação plástica. Em um segundo momento é considerado uma função linear definida por partes para representar o endurecimento/amolecimento característico do maciço, tal como vistos no Capítulo 4.3, contudo em caráter associado ($h = f$) e isotrópico. Essa implementação, é feita exclusivamente através da coesão que dependerá da deformação plástica efetiva ao longo do histórico de tensões (conforme \autoref{funcao_coesao}).
...
\begin{equation}
	...
	\dot q = \dot \lambda \dfrac{\partial f}{\partial q} = \dot \lambda \left( \dfrac{\partial f}{\partial c}\dfrac{\partial c}{\partial \bar \epsilon^p} + \dfrac{\partial f}{\partial \phi}\dfrac{\partial \phi}{\partial \bar \epsilon^p} \right)	
\end{equation}
Como não haverá variação do ângulo de atrito, tem-se $\partial \phi / \partial \bar \epsilon^p = 0$ e apenas a parcela relacionada com a coesão permanece, sendo, seus termos, conforme (\ref{eq:f_Mohr_Coulomb}) e (\ref{eq:f_Drucker_Prager}), dados por:
\begin{equation}
	\label{eq:dfdc}
