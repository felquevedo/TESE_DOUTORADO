...
Quando a função de escoamento não depende de $I_1$   diz-se que a plasticidade é independente da pressão hidrostática, sendo determinada apenas pelo estado de tensões ao longo do plano desviador. Diversas funções de escoamentos podem ser obtidas na literatura, tal como as apresentadas no resumo de \citeonline{Viladkar1995} que reúne tanto as funções de escoamentos clássicas (von-Mises, Tresca, Drucker-Prager, Mohr-Coulomb) quanto mais sofisticadas (\textit{Cap Models, Critical State Model, Desai’s Generlized Model}). O presente trabalho utilizará a função de escoamento de Drucker-Prager (DP) que compreende uma generalização da função de von-Mises (VM) para abarcar a dependência da pressão hidrostática. Essa superfície tem vantagens numéricas pelo fato de não ter vértices e poder aproximar outras superfícies comuns utilizadas para geomateriais (como, por exemplo, Mohr-Coulomb (MC)). (\autoref{Funções_escoamento}).
\begin{figure}[H]
	\begin{center}
